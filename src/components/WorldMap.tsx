...
// 放大时用更大的 canvas + 随机分布光点，避免 tile 重复规律暴露
const SIZE_HI = 256
...
// 每帧生成带流体光效的 pattern 数据
function renderDotPattern(t: number): { width: number; height: number; data: Uint8Array } {
  const ctx = patternCanvas.getContext('2d')!
  ctx.clearRect(0, 0, SIZE, SIZE)

  for (const d of DOT_DEFS) {
    // 每个点独立呼吸 + 流动偏移：用 sin 波叠加模拟流体扰动
    const wave = (Math.sin(t * 1.2 + d.phase) + 1) / 2           // 0~1 呼吸
    const drift = Math.sin(t * 0.7 + d.phase * 1.3) * 1.5        // ±1.5px 漂移
    const px = d.x + drift
    const py = d.y + Math.sin(t * 0.9 + d.phase * 0.8) * 1.2
    const alpha = d.a * (0.3 + wave * 0.7)                        // 亮度随波动

    // 流光颜色在青色 <-> 蓝紫色之间漂移
    const hue = 185 + Math.sin(t * 0.5 + d.phase) * 20           // 165~205
    const sat = 100
    ...
    ctx.beginPath()
    ctx.arc(px, py, d.r * (0.8 + wave * 0.5), 0, Math.PI * 2)
    ctx.fillStyle = core
    ctx.fill()
  }
...
// 放大时使用的高密度随机分布 pattern
function renderHiDotPattern(t: number): { width: number; height: number; data: Uint8Array } {
  ...
  for (const d of HI_DOT_DEFS) {
    ...
    const drift = Math.sin(t * 0.7 + d.phase * 1.3) * 2.5
    const px = d.x + drift
...
function renderFlowPattern(t: number): { width: number; height: number; data: Uint8Array } {
...
export default function WorldMap({ mapState, onCountryClick, lightCountry, onLightDone }: Props) {
  const { lang } = useLang()
